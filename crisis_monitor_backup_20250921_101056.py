    ...
    with open(md_path, "w", encoding="utf-8") as f:
        ...
        if high_risk_indicators:
            ...
            for indicator in high_risk_indicators:
                f.write(f"### {indicator['name']} ({indicator['series_id']})\n")
                f.write(f"- **当前值**: {indicator['current_value']:.3f} \n")
                f.write(f"- **基准值**: {indicator['benchmark_value']:.3f} ({indicator.get('compare_to', 'unknown')})\n")
                f.write(f"- **风险评分**: {indicator['risk_score']:.1f}/100  🔴 高风险\n")
                f.write(f"- **偏离度**: {indicator['current_value'] - indicator['benchmark_value']:.3f}\n")
                f.write(f"- **历史Z分数**: {indicator.get('zscore', 0):.2f}\n")
                f.write(f"- **方向说明**: 该指标{'越高越危险' if indicator.get('higher_is_risk', True) else '越低越危险'}\n")
        ...
        if very_low_risk_indicators:
            f.write("## 🔵 极低风险指标\n\n")
            for indicator in very_low_risk_indicators:
                f.write(f"### {indicator['name']} ({indicator['series_id']})\n")
                f.write(f"- **当前值**: {indicator['current_value']:.3f} \n")
                f.write(f"- **基准值**: {indicator['benchmark_value']:.3f} ({indicator.get('compare_to', 'unknown')})\n")
                f.write(f"- **风险评分**: {indicator['risk_score']:.1f}/100  🔵 极低风险\n")
                f.write(f"- **偏离度**: {indicator['current_value'] - indicator['benchmark_value']:.3f}\n")
                f.write(f"- **历史Z分数**: {indicator.get('zscore', 0):.2f}\n")
                f.write(f"- **方向说明**: 该指标{'越高越危险' if indicator.get('higher_is_risk', True) else '越低越危险'}\n")
                f.write(f"![{indicator['series_id']}](figures/{indicator['series_id']}_latest.png)\n\n")
        ...
        # 指标配置表
        f.write("## 📋 指标配置表\n\n")
        f.write("| 指标名称 | 分组 | 基准分位 | 基准理由 | 变换方法 | 计分方式 |\n")
        f.write("|---------|------|----------|----------|----------|----------|\n")
        ...
        for ind in indicators:
            if ind.get("enabled", True):
                ...
                compare_to = ind.get("compare_to", "unknown")
                transform = ind.get("transform", "level")
                
                # 基准理由
                baseline_reason = ind.get("baseline_reason", "")
                if not baseline_reason:
                    if compare_to.startswith("crisis"):
                        baseline_reason = "危机期基准"
                    elif compare_to.startswith("noncrisis"):
                        baseline_reason = "非危机期基准"
                    else:
                        baseline_reason = "自定义基准"
                
                include_in_score = ind.get("include_in_score", True)
                score_method = "按组平均" if include_in_score else "不计分"
                f.write(f"| {name} | {group} | {compare_to} | {baseline_reason} | {transform} | {score_method} |\n")
        
        f.write("\n")
        
        # 基准分位解释
        f.write("## 📖 基准分位解释\n\n")
        f.write("| 基准分位 | 含义 | 使用场景 |\n")
        f.write("|---------|------|----------|\n")
        f.write("| noncrisis_p25 | 非危机期间25%分位数（较低值） | 收益率曲线倒挂、消费者信心等'越低越危险'指标 |\n")
        f.write("| noncrisis_p75 | 非危机期间75%分位数（较高值） | 利率水平、信用利差等'越高越危险'指标 |\n")
        f.write("| noncrisis_p90 | 非危机期间90%分位数（很高值） | VIX波动率等需要更灵敏捕捉抬升的指标 |\n")
        f.write("| crisis_median | 历史危机期间中位数 | 信用利差、金融状况指数等危机敏感指标 |\n")
        f.write("| crisis_p25 | 历史危机期间25%分位数 | 实体经济指标（就业、GDP等）'越低越危险' |\n")
        f.write("| noncrisis_median | 非危机期间中位数 | 美元指数等中性指标 |\n")
        f.write("\n")
        f.write("**说明**: \n")
        f.write("- p25/p75/p90：表示历史数据中25%/75%/90%的观测值低于该水平\n")
        f.write("- noncrisis：排除历史危机期间的数据，反映'正常'经济环境\n")
        f.write("- crisis：仅使用历史危机期间的数据，反映'危险'水平\n\n")
