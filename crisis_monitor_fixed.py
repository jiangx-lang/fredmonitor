...
修复的问题：
1. 零除错误：当基准值为0时的处理
2. 缺失依赖：requests导入位置优化
3. 数据类型检查：增强空数据处理
4. 错误处理：更细致的异常捕获
5. 内存优化：matplotlib图形资源释放
"""
...
def transform_series(series: pd.Series, transform: str) -> pd.Series:
    ...
    try:
        ...
        if transform == 'level':
            ...
            result = series.pct_change(periods=12) * 100
            return result.replace([np.inf, -np.inf], np.nan)
        elif transform == 'diff':
            return series.diff()
        else:
...
def calculate_risk_score(current_value: float, crisis_stats: Dict[str, float], 
    ...
    try:
        ...
        # 处理基准值为0的情况
        if abs(benchmark) < 1e-10:  # 基准值接近0
            if abs(current_value) < 1e-10:  # 当前值也接近0
                return 50.0  # 中等风险
            else:
                # 基准值为0，当前值不为0
                if higher_is_risk:
...
def save_indicator_plot(ts: pd.Series, title: str, unit: str, crises: List[dict], 
    ...
    try:
        ...
        # 绘制危机期间
        crisis_plotted = False
        for crisis in crises:
            try:
                start_date = pd.to_datetime(crisis['start'])
                end_date = pd.to_datetime(crisis['end'])
                if not crisis_plotted:
