...
def calculate_real_fred_scores(indicators_config=None):
    ...
    if indicators_config:
        ...
        if abs(total_weight - 1.0) > 1e-6:
            print(f"⚠️ 可计分项权重未归一化: {total_weight:.6f} (应为1.0)")
            # 自动归一化
            for ind in scoring_indicators:
                ind['weight'] = ind.get('weight', 0) / total_weight
            print(f"✅ 已自动归一化权重")
        print(f"✅ 配置验证完成: {len(indicators_config)}个指标, 可计分项权重总计: {total_weight:.3f}")
...
def process_single_indicator_real(indicator, crisis_periods):
    ...
    try:
        ...
        # 特殊处理：使用预计算的YoY数据
        if series_id in ['GDP', 'PAYEMS', 'INDPRO', 'NEWORDER', 'CSUSHPINSA', 'TOTALSA']:
            # 使用预计算的YoY数据
            try:
                yoy_file = pathlib.Path(f"data/series/{series_id}_YOY_YOY.csv")
                if yoy_file.exists():
                    ...
                    # 找到与当前日期最接近的YoY数据
                    yoy_ts = parse_numeric_series(yoy_df['yoy_pct'])
                    if not yoy_ts.empty:
                        # 使用最新的YoY值
                        current_value = yoy_ts.iloc[-1]
                        print(f"📊 {series_id}: YoY = {current_value:.2f}% (使用预计算数据)")
                else:
                    print(f"⚠️ {series_id}: YoY数据文件不存在，使用原始值")
            except Exception as e:
                print(f"⚠️ {series_id}: YoY数据读取失败: {e}")
            ...
            try:
                ...
                if ratio_file.exists():
                    ...
                    # 找到与当前日期最接近的比率数据
                    ratio_ts = parse_numeric_series(ratio_df['value'])
                    if not ratio_ts.empty:
                        # 使用最新的比率值
                        current_value = ratio_ts.iloc[-1]
                        print(f"📊 {series_id}: 企业债/GDP比率 = {current_value:.2f}% (使用预计算数据)")
                else:
                    print(f"⚠️ {series_id}: 企业债/GDP比率数据文件不存在，使用原始值")
            except Exception as e:
                print(f"⚠️ {series_id}: 企业债/GDP比率数据读取失败: {e}")
        
        # 计算基准值（使用预计算的YoY基准值）
        benchmark_value = calculate_benchmark_corrected(series_id, indicator, ts)
        
        # 计算风险评分
        risk_score = calculate_risk_score_simple(current_value, benchmark_value, indicator, ts)
        ...
        return {
            ...
            'global_weight': indicator.get('weight', 0),
...
def calculate_benchmark_corrected(series_id, indicator, ts):
    """
    计算修正后的基准值（使用预计算的YoY基准值）
    """
    try:
        # 检查是否有预计算的YoY基准值
        benchmark_file = pathlib.Path("data/benchmarks_yoy.json")
        if benchmark_file.exists():
            ...
            if series_id in benchmarks:
                benchmark = benchmarks[series_id]
                print(f"📊 {series_id}: 使用YoY基准值 {benchmark:.2f}%")
                return benchmark
        
        # 如果没有预计算的基准值，回退到原始方法
        return calculate_benchmark_simple(ts, indicator, {})
        
    except Exception as e:
        print(f"⚠️ {series_id} 基准计算失败: {e}")
        return calculate_benchmark_simple(ts, indicator, {})
