#!/usr/bin/env python3
"""
下载真正的M1货币供应量数据到M1_Components目录
"""
...
CATALOG_FILE = os.path.join(BASE, "config", "real_m1_catalog.yaml")
CATEGORIES_ROOT = pathlib.Path(BASE) / "data" / "fred" / "categories"

def get_m1_category_path(series_id: str) -> pathlib.Path:
    """获取M1数据的保存路径"""
    return CATEGORIES_ROOT / "Monetary_Data" / "M1_Components" / "series" / series_id

def ensure_m1_series_dir(series_id: str) -> pathlib.Path:
    """创建M1序列目录结构"""
    p = get_m1_category_path(series_id)
...
def sync_m1_series(series_config: Dict[str, Any]) -> None:
    """同步单个M1序列"""
    series_id = series_config["id"]
    alias = series_config.get("alias", series_id)
    calc_config = series_config.get("calc")
    
    logger.info(f"开始同步M1序列: {series_id} ({alias})")
    
    try:
        # 创建目录
        folder = ensure_m1_series_dir(series_id)
        
        # 获取元数据
        metadata = fetch_metadata(series_id)
        metadata["alias"] = alias
        metadata["category_id"] = 17
        metadata["subcategory"] = "M1_Components"
        ...
        logger.error(f"✗ 同步M1序列失败 {series_id}: {e}")

def main():
    """主函数"""
    logger.info("开始真正的M1货币供应量数据同步")
    ...
    if not series_list:
        logger.warning("未找到需要同步的M1序列")
        return
    
    logger.info(f"找到 {len(series_list)} 个真正的M1序列需要同步")
    
    # 显示要同步的系列
    logger.info("M1系列列表:")
    for s in series_list:
        logger.info(f"  {s['id']}: {s.get('alias', '')}")
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        # 提交所有任务
        future_to_series = {
            executor.submit(sync_m1_series, series_config): series_config 
        ...
        for future in as_completed(future_to_series):
            ...
            try:
                future.result()
                success_count += 1
                logger.info(f"✓ M1序列 {series_id} 同步完成")
            except Exception as e:
                logger.error(f"✗ M1序列 {series_id} 同步失败: {e}")
    
    logger.info(f"真正的M1货币供应量数据同步完成: {success_count}/{len(series_list)} 成功")
